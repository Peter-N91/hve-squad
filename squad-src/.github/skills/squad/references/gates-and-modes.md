...
5. **council** — `architect`, `security`, `cost-manager`, `product-owner` (+`rai` when relevant) — a `## Council Verdict` in `decisions.md` — a plan artifact exists and pre-implementation stage 3 applies.
6. **implement** — `developer`, or the fan-out specialists — the artifact at each producing role's Deliverable Root — a plan artifact exists; when pre-implementation stage 3 applies, its own recorded `Go` or `Go-With-Conditions` Council Verdict and conditions also exist before implementation.
7. **review** — `tester` — a review record plus its `history/<agent>.md` entry — the implement stage's artifacts exist.

Apply *Pipeline Contract* stage 3 and *Per-Stage Advance Checklist* in `squad-autopilot.instructions.md`, together with [Autopilot-Run Summary](scribe-procedure.md#autopilot-run-summary), to the pre-implementation Council rows and the checklist below. Only a Scribe-recorded trigger assessment and reason in this run's `decisions.md` entry permits the non-applicable no-dispatch exception; confirm that evidence before advancing, without creating a Council Verdict or counting an executed stage. Missing or unjustified applicability evidence blocks the exception. When applicable, a missing verdict blocks implementation and `Stop` fires the Human Gate. The implementation validator-loop Council is unchanged and cannot supply the pre-implementation prerequisite verdict.
