...
| council   | `architect`, `security`, `cost-manager`, `product-owner` (+`rai` when relevant) | a `## Council Verdict` in `decisions.md`  | a plan artifact exists and stage 3 applies             |
| implement | `developer`                                                                    | `.copilot-tracking/changes/*`             | a plan artifact exists; when stage 3 applies, a recorded non-`Stop` Council Verdict with its conditions also exists |
...
For non-applicable pre-implementation Council only, confirm the Scribe-recorded trigger assessment and reason in this run's `decisions.md` entry before advancing. This ordinary decision is the stage's evidence, not a Council Verdict or proof of domain dispatch/consumption. Missing or unjustified applicability evidence blocks this exception. Do not count the documented no-dispatch row as an executed stage. An implementation validator-loop verdict cannot substitute for pre-implementation evidence.

For every required or claimed executed stage, do not advance from stage N to stage N+1 until, for stage N, both are confirmed on disk: (a) the required artifact from the *Artifact Gates* table above, at the owning role's `Deliverable Root`, and (b) this run's `history/<agent>.md` entry for that stage with its consumption block. When either is absent, re-dispatch the owning role or fire the Risk Gate — never advance on assumed completion. State the confirmed evidence (the artifact path and the history entry) when reporting the stage, and quote only paths this run actually enumerated. For a deliverable fan-out run, apply this check per deliverable before the Review stage begins. This restates the Artifact Gates as a mechanical per-stage loop so a lighter model cannot skip a stage by narrating it as done.
...
**The run summary reports the count whether or not anyone reads it.** The Scribe fills the `Dispatch Record` column of `history/autopilot-run-<id>.md` from `history/` for required or claimed executions, and any such stage with no record forces the summary's `Outcome` to `incomplete (<n> stage(s) without a dispatch record)`. Only the documented non-applicable pre-implementation Council row uses `not applicable (no dispatch): <decision ref>` and is excluded from that missing-dispatch count, per *Autopilot-Run Summary* in `references/scribe-procedure.md` of the `squad` skill. The coordinator may not report a run as completed over an `incomplete` summary. This is the check that survives a coordinator which skipped every other one.
...
For a non-applicable pre-implementation Council row, use the recorded reason and no-dispatch representation from *Autopilot-Run Summary* in `references/scribe-procedure.md` of the `squad` skill. A skipped row is not a verdict or a claimed execution; applicable stages retain their own dispatch evidence.
