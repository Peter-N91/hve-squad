...
| `full`         | researcher, lead, developer, tester, challenger, architect, azure-architect, iac-author, deployer, asbuilt-author, azure-diagnose, security, rai, designer, fact-checker, cost-manager, modernizer, prompt-engineer, intake-validator, scribe                   | You want every deployed capability available except the opt-in roles     |
...
An **opt-in role** is a catalog role that no profile seeds, not even `full`. It is fully dispatchable and selectable in a custom roster; it simply never arrives by default, because its work writes somewhere the user may not expect a squad to reach.

`backlog-executor` is the only opt-in role today. It writes into a live Azure DevOps or Jira backlog, where a create is announced to a whole team by notifications, subscriptions, and webhooks the moment it lands. Arriving in a seeded roster would make that reach a surprise, so the role is added deliberately or not at all.

An opt-in role is offered on demand rather than hidden. When a request matches an opt-in role's routing pattern in a squad that does not carry it, the coordinator escalates and **proposes adding the role** — naming what it would be able to write and to which project — instead of silently declining or falling back to a role that cannot do the work. This is the same offer the coordinator makes for `intake-validator` when the intake gate fires in a squad without it. On acceptance the Squad Scribe appends the role to `team.md` and its routing rows to `routing.md`, and the turn continues; on decline the coordinator reports that the plan is complete and the tracker write is the user's to run.

Adding an opt-in role is a roster change like any other: it persists for the project until the user removes it, and the Scribe records the addition so the roster stays the single source of truth.
...
* **product-owner** (GitHub Backlog Manager) — Manages the backlog: triages, refines, and organizes work items. For ADO and Jira this role plans the work items; `backlog-executor` writes them.
* **backlog-executor** (Squad Backlog Executor) — Creates the planned work items in your live Azure DevOps or Jira project, after showing you exactly what it will write and waiting for your approval. Opt-in: no profile includes it, and the squad offers to add it the first time you ask for a tracker write.
