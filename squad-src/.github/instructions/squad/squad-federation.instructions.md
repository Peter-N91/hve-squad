...
## Kind Gate and Advisory Exchange

Read Kind before deriving a member path, checking or repairing a tree, selecting Watch bootstrap/resume, dispatching a single target or fan-out, recovering a producer, or aggregating completion/cost. `in-repo` retains the procedures below. `repo` is advisory-only and never creates, requires, repairs or dispatches `members/<name>/`. Unknown kinds stop. A repo match suggests an explicit exchange; it never issues a packet implicitly. A repo row selected with Watch or autonomous/autopilot mode stops before member access.

For an explicit `exchange=register|send|import|verify`, load `references/repo-exchange.md` from the located `squad` skill and follow its closed v1 contract. Reject combinations with Watch, init, promote or hub autonomy modes. Require an existing federation; do not initialize one as part of exchange. All exchange writes belong to its Scribe, all work/approval stays target-local, and transport is human-only with no network, credentials, clone/fetch, checkout or cwd switching.

Registration confirms a unique name across all kinds and unique canonical GitHub HTTPS URL across repo rows, `Profile=advisory` and human/team Owner, never `watch-mode`. Preserve existing registry/routes; an optional route is suggestion-only with `Parallel-Eligible=no`. It is not Expansion or member Init. Send requires an explicit registered repo alias, task, exact target commit and confirmed request/acceptance criteria. Import yields reported only; Verify requires a separate current human attestation to named evidence and a committed Import.

Repo dependencies remain pending, including blocked consumers. Filter repo rows before local execution eligibility; an empty eligible set is not a completed meta-run. A report or human verification never resumes a consumer automatically. A fresh local user request with separately reviewed local inputs must pass the ordinary gates. Receipt paths are target labels, not files to read at the hub.

Scribe uses `history/repo-exchange/audit.md` for exchange audit, excluded from dispatch and consumption parsing even during recursive enumeration. Preserve a valid local member's flat `history/repo-exchange.md`; normal later dispatch still uses it. No remote consumption, active sub-squads or lifecycle fields are invented in `state.json`. Every positive exchange outcome requires its canonical audit/state/completion seal, not file presence alone.
...
**A healthy federation root holds four core files plus a `history/` directory.** `federation.md`, `meta-routing.md`, `decisions.md`, and `state.json` are seeded when built, promoted or expanded. `history/<sub-squad>.md` is created only for in-repo members and appended on routed turns. `history/autopilot-run-<id>.md` appears only after a meta-run. Optional `exchanges/` and nested exchange audit are not member state.

There is deliberately no `team.md`, `routing.md`, `notifications.md`, `consumption.md`, or `consumption-rates.md` at the federation root; those belong to in-repo members. Their top-level presence is mixed state under *Detection Precedence*. Missing federation history after actual local dispatch is a partial Scribe handoff, not a reason to repair an advisory repo row.
...
2. **Relocate the existing tree.** Apply *Promotion Audit Ownership* in `repo-exchange.md` before moving any file. Retain the complete root `decisions.md` when exchanges or audit exist, with a provenance pointer in new member decisions. Otherwise move decisions intact. Move `team.md`, `routing.md`, `notifications.md`, `state.json`, `consumption.md`, `consumption-rates.md`, and `history/<agent>.md` from `.copilot-tracking/squad/` into `.copilot-tracking/squad/members/<name>/`, preserving contents byte-for-byte apart from the single roster column step 5 rebases. Append-only history and consumption blocks move intact, never edited or truncated. The parameterized squad root keeps the moved files scoped to that member.
...
* Preserve the repository-root `exchanges/` tree, including `exchanges/claims/`, and nested `history/repo-exchange/audit.md` in place during interactive or automatic Promotion. Do not move, copy, reseed or delete claims with member state. Former single-root or removed/moved member bindings stop as `root-relocated` under `repo-exchange.md`; preserve historical evidence and require human review of prior work before a fresh correlation, never same-ID reacceptance.
...
A registry Kind check precedes event-name reuse, profile inference, missing-tree repair and provenance reads: only `in-repo` rows participate. A matching repo row stops; it is never watch-owned or an orphan to repair. Unknown kinds stop. The existing in-repo Watch procedure below is otherwise unchanged.
...
Apply the Kind Gate to both producer and consumer before resolving artifacts. This section's member reads and same-turn recovery apply only to `in-repo`. A repo producer leaves the consumer pending for a fresh local request with reviewed inputs; none of the recovery/assumption cases below overrides that boundary.
...
| Kind        | `in-repo` for local execution; `repo` for explicit advisory exchange only; unknown values stop |
| Location    | `members/<name>/` for `in-repo`; canonical GitHub HTTPS identity for `repo` |
...
For `Kind=repo`, Profile is literal `advisory`, Owner is a confirmed human/team label and the name is an alias, not a directory. The in-repo Init/Expansion flow never seeds repo rows; only explicit Register does. Canonical identity and uniqueness follow *Kind Gate and Advisory Exchange* and `references/repo-exchange.md`.
...
Every sub-squad has a **required, unique name** as the registry key and `squad=<name>` selector, and for `in-repo` only, the `members/<name>/` state directory. Validate names across all kinds before creating a folder or registering an advisory alias; a repo alias never creates a directory.
...
`Kind=repo` adds human-transported advisory packets and receipts, not a cross-repo execution driver. Read `references/repo-exchange.md` from the located `squad` skill for schemas, validator calls, repository-wide claims and committed-operation predicates. A target human opens their own repository and invokes `/squad handoff=<path>`; this hub never opens it or claims its work is verified.
