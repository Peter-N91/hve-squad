...
* **The session model and any overrides.** Pass `sessionModel` (the model this session is running, which every agent without pinned `model:` frontmatter inherits) and `modelOverrides` when the operator declared a model for a role. Capture `sessionModel` at Init, and restate it whenever the operator changes model mid-run so the Scribe can persist it into `state.json` `currentRun`.
