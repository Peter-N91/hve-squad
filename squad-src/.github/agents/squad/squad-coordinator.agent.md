...
6. **Capture the session model.** Ask the user which model this session is running and seed it into `state.json` `currentRun.sessionModel`. Every agent that does not pin `model:` in its own frontmatter inherits this model, so it is the only way the consumption ledger can price those dispatches correctly — an unrecorded session model makes every inherited dispatch fall to a tier fallback and be billed as a cheaper model than the one that actually ran. **Never write the literal string `unknown` into this field to move past the question**: the user is running *some* model and can say which, so ask once rather than degrading silently. State the value back when confirming the build so a wrong answer is visible immediately, and restate it whenever the user switches models mid-run.
...
* **The resolved model and its source.** Resolve it through the ladder in *Model Attribution* in `.github/instructions/squad/squad-state.instructions.md`: an operator declaration, then the dispatched agent's own `model:` frontmatter, then the session model, then `unknown`. Pass `model` and `model_source` together. Never pass a model name you did not resolve — no tier-derived name, no plausible guess. `unknown` is always preferable to a fabricated attribution, because a ledger that names a model the operator never chose invites cost decisions based on a fiction.
* **The session model and any overrides.** Pass `sessionModel` (the model this session is running, which every agent without pinned `model:` frontmatter inherits) and `modelOverrides` when the operator declared a model for a role. Capture `sessionModel` at Init (Phase 1 step 6) and restate it whenever the operator changes model mid-run so the Scribe can persist it into `state.json` `currentRun`. When `state.json` still carries an empty or `unknown` `sessionModel` on a later turn, ask the user for it before dispatching rather than passing the gap forward — every turn that runs without it prices every inherited dispatch as a cheaper model than the one that actually ran.
