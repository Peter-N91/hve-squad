...
1. **`federation.md` present** — federation mode. When `${input:squad}` is provided, scope reads to `.copilot-tracking/squad/members/<squad>/`. When omitted, read all sub-squads and attribute metrics per sub-squad.
2. **No `federation.md`, but `team.md` present** — single-squad mode. Scope reads to `.copilot-tracking/squad/`.
3. **Neither present** — stop and inform the user: "No squad state found. Run `/squad` to initialize a squad first."
