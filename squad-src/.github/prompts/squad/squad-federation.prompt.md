...
* ${input:mode}: (Optional) The autonomy mode (`autonomous` or `autopilot`) forwarded to the selected sub-squad's coordinator run. When omitted, the sub-squad uses the standard interactive tiers.
...
4. Forward `${input:profile}`, `${input:tier}`, `${input:owner}`, and `${input:mode}` as pass-through hints to the selected sub-squad's coordinator run.
